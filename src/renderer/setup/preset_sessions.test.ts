...
describe('preset session backfill', () => {
  it('includes the shared ChatBridge live fixtures in the preset bootstrap list', () => {
    expect(getChatBridgePresetSessionBundles('en').map(({ session }) => session.name)).toEqual([
      ...
      `${CHATBRIDGE_LIVE_SEED_PREFIX} Flashcard Studio Google Sheets resume`,
      `${CHATBRIDGE_LIVE_SEED_PREFIX} Flashcard Studio Google Sheets denied reconnect`,
      `${CHATBRIDGE_LIVE_SEED_PREFIX} Flashcard Studio Google Sheets expired auth`,
