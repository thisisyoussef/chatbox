...
- Data flow summary:
  Host creates appInstance -> mints launch-scoped bridgeSession -> bootstraps runtime with signed envelope and dedicated channel -> runtime acks -> host validates later events against the active session.

## Architecture Decisions

- Decision:
  Adopt a launch-scoped, bound-channel bridge session rather than ambient cross-window messaging.
- Alternatives considered:
- Origin checks only
- Iframe-local secrets without host session binding
- Rationale:
  The trust boundary is session-specific, not just origin-specific, and later audit/recovery paths also benefit from explicit session metadata.
