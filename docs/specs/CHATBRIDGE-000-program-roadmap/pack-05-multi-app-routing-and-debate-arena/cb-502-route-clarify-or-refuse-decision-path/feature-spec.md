...
The platform needs explicit behavior when a prompt maps to one app, several apps, or no app at all. Without this, multi-app routing will feel arbitrary and unsafe.
...
- How this story contributes to the pack: The host can choose between invoke, clarify, or refuse states explicitly.

## User Stories

- As a user, I want the assistant to ask when my request is ambiguous instead of guessing the wrong app.
- As the host, I want clear refusal behavior when a prompt should remain chat-only.

## Acceptance Criteria

- [ ] AC-1: The host can choose between invoke, clarify, or refuse states explicitly.
- [ ] AC-2: Ambiguous prompts surface a user-facing clarifier instead of a silent guess.
- [ ] AC-3: Unrelated prompts stay in chat-only mode without forced app routing.
...
- Tenant policy precedence resolution
- Story Builder auth flow
